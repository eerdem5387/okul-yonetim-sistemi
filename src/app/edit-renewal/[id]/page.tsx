...
export default function EditRenewalPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchContract = useCallback(async () => {
    ...
    try {
      ...
      if (!fetchedStudentId) {
        console.error("[Edit Renewal] Student ID not found")
        setLoading(false)
        return
      }
      ...
      if (!studentData) {
        console.error("[Edit Renewal] Student data not found")
        setLoading(false)
        return
      }
    ...
    <div className="p-6">
      <div className="mb-6">
        <div className="flex items-center gap-4 mb-4">
          <Link href="/renewal/list">
            <Button variant="outline" size="sm">
              <ArrowLeft className="h-4 w-4 mr-2" />
              Geri Dön
            </Button>
          </Link>
          <h1 className="text-3xl font-bold text-gray-900">Kayıt Yenileme Sözleşmesi Düzenle</h1>
        </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex gap-4">
          ...
          <Link href="/renewal/list" className="flex-1">
