...
export default function EditUniformPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchContract = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        console.error("[Edit Uniform] Failed to fetch contract:", response.status)
        setLoading(false)
        return
      }
      
      const data = await response.json()
      console.log("[Edit Uniform] Fetched contract data:", data)
      
      // Öğrenci ID'sini al ve sakla
      const fetchedStudentId = data.studentId
      if (!fetchedStudentId) {
        console.error("[Edit Uniform] Student ID not found in contract")
        setLoading(false)
        return
      }
      ...
      if (!studentData) {
        console.error("[Edit Uniform] Student data not found in contract")
        setLoading(false)
        return
      }
      
      console.log("[Edit Uniform] Student data from API:", studentData)
      ...
      console.log("[Edit Uniform] Updated contract data:", updatedContractData)
