/**
 * Beden Eğitimi Katılım Sertifikası
 * Certificate of Physical Education Participation — English PDF
 *
 * Dinamik metin: yıllık beden eğitimi programı + puan /100 + genel başarı seviyesi.
 */

export interface BedenEgitimParticipant {
  firstName: string
  lastName: string
  tcNumber: string
  grade: string
...
export interface BedenEgitimCertData {
  title: string
  ...
  createdAt: string
  participants: BedenEgitimParticipant[]
}
...
function getAttainmentLevel(score: number): string {
...
function participantPage(data: BedenEgitimCertData, participant: BedenEgitimParticipant, pageIndex: number): string {
  ...
  <div class="cert-page">
    ...
    <div class="cert-header">
      ...
      <div class="cert-school-info">
        ...
        <div class="cert-school-subtitle">IB Programme · Physical Education</div>
      </div>
    </div>
    ...
    <div class="cert-badge-row">
      <div class="cert-badge">CERTIFICATE OF PHYSICAL EDUCATION PARTICIPATION</div>
    </div>

    <div class="cert-title-block">
      <h1 class="cert-main-title">Physical Education Programme Participation</h1>
    ...
    <div class="cert-body">
      ...
      <div class="cert-details-grid">
        ...
        <div class="cert-detail-item">
          <span class="cert-detail-label">Programme Period</span>
      ...
      <div class="cert-achievement-box">
        <p class="cert-achievement-text">
          The participant has successfully completed the Levent College IB physical education annual
          curriculum programme and achieved a score of <strong>${participant.score}</strong> out of
          <strong>100</strong>.
          Based on holistic assessment of participation, effort, and skill development across the
          programme units, their overall physical education attainment level has been determined as
          <strong>${level}</strong>.
        </p>
      </div>
    ...
    <div class="cert-cert-no">
      Certificate No: LK-PE-${certSuffix}
    </div>
  </div>`
}

export function buildBedenEgitimCertificateHTML(data: BedenEgitimCertData): string {
  const pages = data.participants
    .map((p, i) => participantPage(data, p, i))
    .join("\n")
...
<head>
  ...
  <title>Certificate of Physical Education Participation</title>
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .cert-divider-bottom {
      height: 2px;
      background: linear-gradient(90deg, #065f46 0%, #34d399 50%, #065f46 100%);
      margin: 12px 0 8px;
      border-radius: 2px;
      position: relative;
      z-index: 1;
    ...
    .cert-info-value { font-size: 12px; font-weight: 500; color: #064e3b; }
    ...
    .lvl-excellent     { color: #065f46; background: #ecfdf5; border-color: #047857; }
    .lvl-very-good     { color: #1d4ed8; background: #eff6ff; border-color: #2563eb; }
    .lvl-good          { color: #047857; background: #ecfdf5; border-color: #059669; }
    ...
    .cert-sign-name { font-size: 12px; font-weight: 600; color: #065f46; }
