...
export default function RenewalPage() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const response = await fetch(`/api/pdf/renewal/${selectedStudent.id}`)
      if (response.ok) {
        const blob = await response.blob()
        const url = window.URL.createObjectURL(blob)
        const a = document.createElement("a")
        a.href = url
        a.download = `kayit-yenileme-${selectedStudent.firstName}-${selectedStudent.lastName}.pdf`
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-2">Mevcut öğrenci kayıt yenileme sözleşmesini oluşturun</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Kayıt Yenileme Sözleşmesi</CardTitle>
            <CardDescription>
              {selectedStudent 
