/**
 * Basketbol Eğitimi Katılım Sertifikası
 * Certificate of Basketball Education Participation — English PDF
 *
 * Dinamik metin (puan + performans seviyesi):
 * "The participant has successfully completed the Levent College IB basketball
 * annual training programme and achieved a score of [X] out of 100. Based on
 * this evaluation, their performance level has been determined as [LEVEL]."
 */

export interface BasketbolEgitimParticipant {
  firstName: string
  lastName: string
  tcNumber: string
  grade: string
...
export interface BasketbolEgitimCertData {
  title: string
  ...
  createdAt: string
  participants: BasketbolEgitimParticipant[]
}
...
/** Puan → performans seviyesi (basketbol / spor değerlendirmesi) */
...
function participantPage(data: BasketbolEgitimCertData, participant: BasketbolEgitimParticipant): string {
  ...
  <div class="cert-page">
    ...
    <div class="cert-header">
      ...
      <div class="cert-school-info">
        ...
        <div class="cert-school-subtitle">IB Programme · Basketball</div>
      </div>
    </div>
    ...
    <div class="cert-badge-row">
      <div class="cert-badge">CERTIFICATE OF BASKETBALL EDUCATION PARTICIPATION</div>
    </div>

    <div class="cert-title-block">
      <h1 class="cert-main-title">Basketball Training Participation</h1>
    ...
    <div class="cert-body">
      ...
      <div class="cert-achievement-box">
        <p class="cert-achievement-text">
          The participant has successfully completed the Levent College IB basketball annual training
          programme and achieved a score of <strong>${participant.score}</strong> out of
          <strong>100</strong>.
          Based on this evaluation, their performance level has been determined as
          <strong>${level}</strong>.
        </p>
      </div>
    ...
    <div class="cert-cert-no">
      Certificate No: LK-BB-${Date.now().toString(36).toUpperCase().slice(-6)}
    </div>
  </div>`
}

export function buildBasketbolEgitimCertificateHTML(data: BasketbolEgitimCertData): string {
...
<head>
  ...
  <title>Certificate of Basketball Education Participation</title>
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .cert-divider-top {
      height: 3px;
      background: linear-gradient(90deg, #9a3412 0%, #f97316 50%, #9a3412 100%);
      margin: 8px 0;
      border-radius: 2px;
      position: relative;
      z-index: 1;
    ...
    .cert-divider-bottom {
      height: 2px;
      background: linear-gradient(90deg, #9a3412 0%, #fb923c 50%, #9a3412 100%);
      margin: 12px 0 8px;
      border-radius: 2px;
      position: relative;
      z-index: 1;
    ...
    .cert-info-value { font-size: 12px; font-weight: 500; color: #431407; }
    ...
    .lvl-excellent     { color: #9a3412; background: #fff7ed; border-color: #c2410c; }
    .lvl-very-good     { color: #1d4ed8; background: #eff6ff; border-color: #2563eb; }
    .lvl-good          { color: #c2410c; background: #fff7ed; border-color: #ea580c; }
    ...
    .cert-sign-name { font-size: 12px; font-weight: 600; color: #9a3412; }
