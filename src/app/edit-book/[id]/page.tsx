...
export default function EditBookPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    const extractIdFromPath = async () => {
      try {
        ...
        console.log("[Edit Book] Contract ID from URL:", id, "Pathname:", pathname)
        
        // Her zaman ID'yi güncelle - pathname değiştiğinde veya mount olduğunda
        if (id && isMounted) {
          console.log("[Edit Book] Setting contract ID:", id)
        ...
        console.error("[Edit Book] Error extracting ID:", error)
      }
    }
  ...
  const fetchContract = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        console.error("[Edit Book] Failed to fetch contract:", response.status)
        setLoading(false)
        return
      }
      
      const data = await response.json()
      console.log("[Edit Book] Fetched contract data:", data)
      
      // Öğrenci ID'sini al ve sakla
      const fetchedStudentId = data.studentId
      if (!fetchedStudentId) {
        console.error("[Edit Book] Student ID not found in contract")
        setLoading(false)
        return
      }
      ...
      if (!studentData) {
        console.error("[Edit Book] Student data not found in contract")
        setLoading(false)
        return
      }
      
      console.log("[Edit Book] Student data from API:", studentData)
      ...
      console.log("[Edit Book] Updated contract data:", updatedContractData)
    ...
    <div className="p-6">
      <div className="mb-6">
        <div className="flex items-center gap-4 mb-4">
          ...
          <h1 className="text-3xl font-bold text-gray-900">Kitap Sözleşmesi Düzenle</h1>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>KİTAP SÖZLEŞMESİ</CardTitle>
          <CardDescription>
          ...
          <div className="border-t pt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input
                  id="bookSet"
                  value={contract.bookSet}
                  onChange={(e) => handleInputChange("bookSet", e.target.value)}
                />
              </div>
              <div>
