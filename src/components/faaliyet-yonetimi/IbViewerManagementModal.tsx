...
export function IbViewerManagementModal({ onClose }: { onClose: () => void }) {
  ...
  const fetchViewers = async () => {
    try {
      const res = await fetch("/api/ib-viewers", { headers: staffAuthHeaders() })
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/ib-viewers/${id}`, {
        method: "DELETE",
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-gray-100 bg-white p-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Dış IB personeli /ib-viewer üzerinden yalnızca onaylı faaliyetleri görüntüler.
            </p>
          </div>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                      <Label>
                        Şifre *{" "}
                        {editingViewer && (
                          <span className="text-xs text-gray-500">(Değiştirmek için doldurun)</span>
                        )}
                      </Label>
                      <Input
                        type="password"
