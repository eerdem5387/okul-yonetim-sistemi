...
export default function EditMealPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchContract = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        console.error("[Edit Meal] Failed to fetch contract:", response.status)
        setLoading(false)
        return
      }
      
      const data = await response.json()
      console.log("[Edit Meal] Fetched contract data:", data)
      
      // Öğrenci ID'sini al ve sakla
      const fetchedStudentId = data.studentId
      if (!fetchedStudentId) {
        console.error("[Edit Meal] Student ID not found in contract")
        setLoading(false)
        return
      }
      ...
      if (!studentData) {
        console.error("[Edit Meal] Student data not found in contract")
        setLoading(false)
        return
      }
      
      console.log("[Edit Meal] Student data from API:", studentData)
      ...
      console.log("[Edit Meal] Updated contract data:", updatedContractData)
