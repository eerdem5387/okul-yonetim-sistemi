/**
 * LEVENT COLLEGE IB PROGRAMME — CERTIFICATE OF TOURNAMENT PARTICIPATION
 */

import { getPrincipalByGrade } from "@/lib/activity-types-config"

export interface TurnuvaKatilimParticipant {
  firstName: string
  lastName: string
  tcNumber: string
  grade: string
...
export interface TurnuvaKatilimCertData {
  tournamentDescription: string
  startDate: string
  endDate: string
  teacherName: string
  createdAt: string
  participants: TurnuvaKatilimParticipant[]
}
...
const BODY_TEXT =
  "This document certifies that the student has participated in the tournament, and the evidence included in the attached file verifies their own effort, and performance throughout the process."

function page(data: TurnuvaKatilimCertData, p: TurnuvaKatilimParticipant, idx: number): string {
  ...
  <div class="cert-page">
    ...
    <div class="cert-body">
      <p class="cert-intro">${BODY_TEXT}</p>
    ...
    <div class="cert-foot">Certificate Ref: LK-TP-${suffix}</div>
  </div>`
}

export function buildTurnuvaKatilimCertificateHTML(data: TurnuvaKatilimCertData): string {
...
<head>
  <meta charset="UTF-8" />
  <title>Certificate of Tournament Participation</title>
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .cert-doc-title { font-size: 11px; font-weight: 700; color: #115e59; letter-spacing: 1.2px; margin-top: 4px; }
    .cert-doc-subtitle { font-size: 10px; font-weight: 600; color: #0d9488; margin-top: 2px; letter-spacing: 0.8px; }
    .cert-divider { height: 2px; background: linear-gradient(90deg, #0f766e, #2dd4bf, #0f766e); margin: 12px 0 18px; }
    .cert-body { color: #1e293b; }
    .cert-intro {
      font-size: 12.5px; line-height: 1.7; text-align: justify; margin-bottom: 22px;
      padding: 14px 16px; background: #f0fdfa; border-left: 4px solid #14b8a6; border-radius: 0 8px 8px 0;
    }
    ...
    .sig-seal {
      height: 64px; margin: 8px auto 8px; max-width: 64px; border: 2px dashed #0d9488; border-radius: 50%;
      display: flex; align-items: center; justify-content: center; font-size: 8px; font-weight: 700; color: #0f766e;
    }
