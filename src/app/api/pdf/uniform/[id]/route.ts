...
export async function GET(
    ...
    try {
        ...
        const html = generateContractHTML(contractData, 'Forma Sözleşmesi')
        const pdf = await generatePDF(html)

        return new NextResponse(pdf, {
            headers: {
                'Content-Type': 'application/pdf',
                'Content-Disposition': `attachment; filename="forma-sozlesmesi-${contract.student.firstName}-${contract.student.lastName}.pdf"`
            }
        })
    } catch (error) {
